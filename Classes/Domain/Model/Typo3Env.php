...
namespace Barlian\Domain\Model;
...
class Typo3Env {
	...
	public function initTSFE($pid = 1, $ws = 0) {
		...
		if (count($page) == 0 && $GLOBALS['BE_USER']->workspace != 0) {
			...
			$wsRec = \TYPO3\CMS\Core\Utility\BackendUtility::getWorkspaceVersionOfRecord(
				$GLOBALS['BE_USER']->workspace, 'pages', $pid
			);
	...
	public static function getExtBackPath($extKey = 'imagemap_wizard') {
		return self::getBackPath() . str_replace(PATH_site, '', ExtensionManagementUtility::extPath($extKey));
	}
